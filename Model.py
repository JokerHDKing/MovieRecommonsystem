...
from torch.nn import Embedding, Linear, Conv2d, init

from MovieLen import MovieLen
...
class Model(nn.Module):
    def __init__(self, use_mov_title, use_mov_cat, use_age_job, fc_sizes):
        ...
        # 对电影名称做映射（通过卷积和全相加来提取特征，然后通过步长降低维度）
        ...
        self._user_layers = nn.ModuleList()
        for i in range(len(self.fc_sizes)):
            linear = nn.Linear(
                in_features=user_sizes[i],
                out_features=user_sizes[i + 1])
            self.add_module('linear_user_%d' % i, linear)
            self._user_layers.append(linear)
            if acts[i] == 'relu':
                act = nn.ReLU()
                self.add_module('user_act_%d' % i, act)
                self._user_layers.append(act)
            # 设置参数的初始化方式
            init.normal_(linear.weight, mean=0.0, std=1.0 / math.sqrt(user_sizes[i]))

        # 电影特征和用户特征使用了不同的全连接层，不共享参数
        movie_sizes = [200] + self.fc_sizes
        acts = ["relu" for _ in range(len(self.fc_sizes))]
        self._movie_layers = nn.ModuleList()
        for i in range(len(self.fc_sizes)):
            linear = nn.Linear(
                in_features=movie_sizes[i],
                out_features=movie_sizes[i + 1])
            setattr(self, 'linear_movie_%d' % i, linear)
            self._movie_layers.append(linear)
            if acts[i] == 'relu':
                act = nn.ReLU()
                setattr(self, 'movie_act_%d' % i, act)
                self._movie_layers.append(act)
            # 设置参数的初始化方式
            init.normal_(linear.weight, mean=0.0, std=1.0 / math.sqrt(movie_sizes[i]))
    ...
    def get_usr_feat(self, usr_var):
        ...
        user_features = torch.tanh(self.usr_combined(usr_feat))

        #通过3层全链接层，获得用于计算相似度的用户特征
    ...
    def get_mov_feat(self, mov_var):
        ...
        if self.use_mov_title:
            # 计算电影名字的特征映射，对特征映射使用卷积计算最终的特征（-1可用于自动计算改为都大小）
            ...
            mov_title = mov_title.view(batch_size, -1)
            feats_collect.append(mov_title)
        # 使用一个全连接层，整合所有电影特征，映射为一个200维的特征向量

        mov_feat = torch.concat(feats_collect, dim=1)
        mov_features = torch.tanh(self.mov_concat_embed(mov_feat))
    ...
    def forward(self, usr_var, mov_var):
        ...
        sim = F.cosine_similarity(user_features, mov_features, dim=1).reshape([-1, 1])
        res = torch.mul(sim, 5.0)
